...
const ModelCategoryRankings: React.FC<ModelCategoryRankingsProps> = ({ initialExpanded = false }) => {
  ...
  useEffect(() => {
    const fetchModelRankings = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch model rankings: ${response.status} ${response.statusText}`);
        }
        ...
        console.error('Error fetching model rankings:', err);
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <ModelTrainingIcon sx={{ mr: 1, color: theme.palette.primary.main }} />
      ...
      {/* Collapsible Content */}
      <Collapse in={expanded} timeout="auto" sx={{ flexGrow: 1, overflow: 'auto' }}>
        <Box 
          sx={{ 
            p: 2, 
            bgcolor: 'white',
        ...
        >
          {loading ? (
            ...
            Object.entries(modelRankings).map(([category, data], index) => (
              <Box key={category} sx={{ mb: index < Object.keys(modelRankings).length - 1 ? 3 : 0 }}>
                  ...
                  {data.models.map((model, modelIndex) => (
                    <Box
                      ...
                      sx={{
                        ...
                        bgcolor: model.primary ? alpha(getCategoryColor(category), 0.05) : 'transparent',
