...
const API_BASE_URL =
  ...
  'http://localhost:8000'; // Remove trailing slash if present
...
const ModelSelectionChatView: React.FC<ModelSelectionChatViewProps> = ({
  session,
  apiClient,
}) => {
        ...
        if (eventData.event === 'metadata' && eventData.data) {
          ...
          setMessages(prev => {
            return prev.map(msg => {
              if (msg.id === targetAssistantId) {
                ...
                const newTextPortion = eventData.data;
          ...
          throw new Error(
            eventData.data.detail ||
              ...
              'Unknown server error in stream'
          );
        } else if (eventData.event === 'end_stream') {
        }
      } catch (e) {
        // Error processing event block - silently continue
      }
    };
  ...
  return (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Typography variant="body2" color="text.secondary" paragraph>
        Enter a prompt and see how it's classified and which AI model is
        ...
        {messages.length === 0 ? (
          ...
          >
            ...
            <Typography variant="body2" sx={{ mt: 1 }}>
              Try different types of prompts like creative writing, factual
              questions, coding problems, math questions, or logical reasoning
            </Typography>
          </Box>
        ) : (
                  ...
                  {message.sender === 'assistant' && message.modelInfo && (
                    <>
                      ...
                      <Box sx={{ mt: 1 }}>
                        ...
                        {message.modelInfo.allCategories &&
                          Object.keys(message.modelInfo.allCategories).length >
                            0 && (
                            <>
                              <Typography
                                variant="caption"
